...
def model_dir(model_info, tmpdir_factory):
    ...
    for dir_input in dirs_input:
        ...
        res = subprocess.run(f'python cli.py train --model {model_info["model"]} --dataroot {dir_input} --name . --batch-size 1 --num-threads 0 --checkpoints-dir {dir_save} --remote True --n-epochs 1 --n-epochs-decay 1',shell=True)
