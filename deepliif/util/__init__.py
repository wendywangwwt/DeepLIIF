...
def allowed_file(filename,image_extensions=['.png', '.jpg', '.tif', '.jpeg', '.svs'],
...
def test_diff_tensor(ts1,ts2,ts1_name=None,ts2_name=None,threshold=10,verbose=0):
    ...
    if verbose > 1:
        ...
        print(ts1.shape)
        print(ts1[0, 0:10])
        print('min abs value:{}'.format(torch.min(torch.abs(ts1))))
        
        print(f'{ts2_name}:')
        print(ts2.shape)
        print(ts2[0, 0:10])
        print('min abs value:{}'.format(torch.min(torch.abs(ts2))))
