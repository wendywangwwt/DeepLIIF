...
DEFAULT_OPTIONS = {'concateToWideFormat':
                      {'input_img':
                          {'input_mod':['_ihc.png','{input_dir}'],
                           'trans_mod':['_Marker.png','{input_dir}'],
                   ...
                   'trainTestSplit':
                      {'train_ratio':0.8,
                       'output_dir':'{outpout_dir}_traintest{train_ratio}',
                       'img_names_test':[], # if this list is provided, random split with train ratio will be ignored
...
class prepareDataPipeline():
    def __init__(self, input_dir, output_dir, steps, seg_gen, options):
        ...
        opt = {k:v for k,v in DEFAULT_OPTIONS.items() if k in steps}
        opt = modify_options(opt, options)
    ...
    def concateToWideFormat(self, opt):
        ...
        # obtain tile size
        path_img_check = os.path.join(l_input_config[0][1],f'{img_names_common[0]}{l_input_config[0][0]}')
            ...
            for i, (suffix, input_dir) in enumerate(l_input_config):
                ...
                if i in opt['process_mod'].keys():
                    if opt['process_mod'][i] == 'black':
                        ...
                        img_tile = clean_stains_for_image(img_tile)
                    else:
    ...
    def trainTestSplit(self,opt):
        ...
        for img_name in img_names:
            img = Image.open(os.path.join(self.input_data,f'{fn}.png'))
            if len(opt['process_mod']) > 0:
                for i,process_type in opt['process_mod']:
                    if opt['process_mod'][i] == 'black':
                        img_tile = Image.new('RGB', (w,h)) # default is black
                    elif opt['process_mod'][i] == 'cleanStains':
                        img_tile = img.crop((w*i, 0, w*(i+1), h))
                        img_tile = clean_stains_for_image(img_tile)
                    else:
                        raise Exception(f"Not implemented: process type {opt['process_mod'][i]}")
                    img.paste(img_tile,(w*i,0))
            ...
            img_input = img.crop((0, 0, w*input_no, h))
    ...
    def augmentTrainSet(self,opt):
        input_dir = self.input_dir if not hasattr(self,'train_dir') else self.train_dir
        output_dir = opt['output_dir'].format(self.output_dir)
        
        from Image_Processing_Helper_Functions import augment_set
...
if __name__ == '__main__':
    ...
    parser.add_argument('--steps',type=str,default='concateToWideFormat,trainTestSplit,augmentTrainSet,calculateValMetric,addExistingData',
