...
DEFAULT_OPTIONS = {'concateToWideFormat':
                      {'input_img':
                          {'input_mod':[['_ihc.png','{input_dir}']],
                           #'trans_mod':[['_ihc_mod4-Marker.png','{input_dir}']],
                           # 'trans_mod':[['_ihc_mod1-Hema.png','{input_dir}'],
                           #               ['_ihc_mod2-DAPI.png','{input_dir}'],
                           #               ['_ihc_mod3-Lap2.png','{input_dir}'],
                           #               ['_ihc_mod4-Marker.png','{input_dir}']],
                           'seg_mod':[['_seg.png','{input_dir}']]},
                       'output_dir':'{output_dir}',
                       'process_mod':{}, # {<index in ALL mod>:<process type>}; currently support either cleanStains or black
                       }, 
                   'trainTestSplit':# NOTE: this steps include createCLIInput and createGT
                      {'trainValTest_ratio':"0.8,0,0.2",
                       'output_dir':'{output_dir}_traintest{train_ratio}',
                       'img_names_val':[], # if this list is provided and test ratio is 0 (or if both are provided), random split with train ratio will be ignored
                       'img_names_test':[], # if this list is provided and val ratio is 0 (or if both are provided), random split with train ratio will be ignored
                   ...
                   'createCLIInput': # whether to take the input tiles and put into subfolder (e.g., val_cli)
                      {'output_dir':'{input_dir}_cli',
                       'input_no':1,
                      },
                   'createGT': # whether to take the last tile and put into subfolder (e.g., val_cli_gt)
                      {'output_dir':'{input_dir}_cli_gt'
                      },
                   'augmentTrainSet':
                      {'output_dir':'{output_dir}_aug', # if trainTestSplit is part of the pipeline, then augmenTrainSet writes the aug img to the same train subfolder instead of a new folder {output_dir}_aug
                       'aug_no':2,
                       'modality_types':['IHC','Seg'],
                       #'modality_types':['IHC','Marker','Seg'],
                       # 'modality_types':['IHC','Hema','DAPI','Lap2','Marker','Seg'],
...
class prepareDataPipeline():
    def __init__(self, input_dir, output_dir, steps, seg_gen, options, subset):
        ...
        if subset is not None:
            ...
            with open(subset, 'r') as f:
                self.subset = [x[:-1] for x in f.readlines() if x != '']
        
        if 'concateToWideFormat' in self.steps:
            opt = self.opt['concateToWideFormat']
            # process input config list
            l_input_config = opt['input_img']['input_mod']
            if 'trans_mod' in opt['input_img']:
              l_input_config += opt['input_img']['trans_mod']
            l_input_config += opt['input_img']['seg_mod']
            self.l_input_config = l_input_config
            ...
            self.img_names = sorted(img_names_common)
            if self.subset is not None:
                img_names_include = [fn.replace('.png','') for fn in self.subset]
                img_names_common = [img_name for img_name in img_names_include if img_name in self.img_names]
                self.img_names = sorted(img_names_common)
                print(f'The provided subset list contains {len(img_names_include)} image names, among which {len(img_names_common)} are located to proceed with')
    ...
    def _get_img_size(self,path_img,update_tile_size=True):
        img = Image.open(path_img)
        if update_tile_size:
            self.tile_size = img.size # each image is a square tile (not wide concatenated images)
    ...
    def concateToWideFormat(self, opt):
        ...
        l_input_config = self.l_input_config
        img_names_common = self.img_names
        
        # obtain tile size
        path_img = os.path.join(l_input_config[0][1],f'{img_names_common[0]}{l_input_config[0][0]}')
        w, h = self._get_img_size(path_img)
    ...
    def trainTestSplit(self,opt):
        # print(opt['trainValTest_ratio'])
        train_ratio, val_ratio, test_ratio = opt['trainValTest_ratio']#[float(ratio) for ratio in opt['trainValTest_ratio'].split(',')]
        assert train_ratio + val_ratio + test_ratio == 1, f"trainValTest_ratio does not all up to 1 ({opt['trainValTest_ratio']})"
        
        output_dir = opt['output_dir'].format(output_dir=self.output_dir,train_ratio='')
        ...
        if val_ratio > 0:
            ...
            output_dir_val_cli = os.path.join(output_dir,'val_cli')
            ensure_exists(output_dir_val)
            ensure_exists(output_dir_val_cli)
            self.val_dir = output_dir_val
        if test_ratio > 0:
            output_dir_test = os.path.join(output_dir,'test')
            output_dir_test_cli = os.path.join(output_dir,'test_cli')
            ensure_exists(output_dir_test)
            ensure_exists(output_dir_test_cli)
            self.test_dir = output_dir_test
        
        output_dir_val_cli_gt = None
        output_dir_test_cli_gt = None
        if self.seg_gen and opt['create_gt']:
            if val_ratio > 0:
                output_dir_val_cli_gt = os.path.join(output_dir,'val_cli_gt')
                ensure_exists(output_dir_val_cli_gt)
            if test_ratio > 0:
                output_dir_test_cli_gt = os.path.join(output_dir,'test_cli_gt')
        ...
        if len(opt['img_names_test']) == 0 and len(opt['img_names_val']) == 0:
            #self.img_names = sorted(self.img_names)
            random.seed(0)
            print('Train-test split with train/val/test ratio',train_ratio,val_ratio,test_ratio)
            self.img_names_train = random.sample(self.img_names,k=int(len(self.img_names)*train_ratio))
            img_names_rest = sorted([e for e in self.img_names if e not in self.img_names_train])
            print('img_names_rest:',img_names_rest[:5])
            self.img_names_val = random.sample(img_names_rest,k=int(len(self.img_names)*val_ratio))
            self.img_names_test = [e for e in self.img_names if e not in self.img_names_train+self.img_names_val]
        else:
            print(f"Train-test split with provided val set of size test set of size {len(opt['img_names_val'])} and test set of size {len(opt['img_names_test'])}",)
            self.img_names_val = [e for e in self.img_names if e in opt['img_names_val']]
            self.img_names_test = [e for e in self.img_names if e in opt['img_names_test']]
            self.img_names_train = [e for e in self.img_names if e not in self.img_names_test+self.img_names_val]
        print('Train, val, test set size:', len(self.img_names_train), len(self.img_names_val), len(self.img_names_test))
        ...
        for img_name in self.img_names:
            ...
            if img_name in self.img_names_test:
                ...
                if output_dir_test_cli_gt is not None:
                    img.save(os.path.join(output_dir_test_cli_gt,f'{img_name}.png')) # this is needed for seg metric calculation
            ...
            elif img_name in self.img_names_val:
                img.save(os.path.join(output_dir_val,f'{img_name}.png'))
                img_input = img.crop((0, 0, w*opt['input_no'], h))
                img_input.save(os.path.join(output_dir_val_cli,f'{img_name}.png'))
                if output_dir_val_cli_gt is not None:
                    img.save(os.path.join(output_dir_val_cli_gt,f'{img_name}.png')) # this is needed for seg metric calculation
                    img_seg = img.crop((img.size[0]-w, 0, img.size[0], h)) # the last tile
                    img_seg.save(os.path.join(output_dir_val_cli_gt,f'{img_name}_SegRefined.png'))
            else:
    ...
    def augmentTrainSet(self,opt):
        ...
        augment_set(input_dir=input_dir, 
                    output_dir=output_dir, 
                    aug_no=opt['aug_no'], 
                    modality_types=opt['modality_types'], 
                    nearest_interpolation=opt['nearest_interpolation'])
        
        # now this output directory becomes the input for the next step in the pipeline
        #self.input_dir = output_dir
    
    
    def createCLIInput(self,opt):
        """
        This one is used without train/test split, e.g., when the provided input dir is already a validation split.
        """
        output_dir_cli = opt['output_dir'].format(input_dir=self.input_dir)
        ensure_exists(output_dir_cli)
        
        w = h = None
        for img_name in self.img_names:
            if w is None:
                w, h = self._get_img_size(os.path.join(self.input_dir,f'{img_name}.png'),update_tile_size=False)
            img = Image.open(os.path.join(self.input_dir,f'{img_name}.png'))
            img = img.crop((0,0,h*opt['input_no'],h))
            img.save(os.path.join(output_dir_cli,f'{img_name}.png'))
        
      
    def createGT(self,opt):
        """
        This one is used without train/test split, e.g., when the provided input dir is already a validation split.
        """
        output_dir_cli_gt = opt['output_dir'].format(input_dir=self.input_dir)
        ensure_exists(output_dir_cli_gt)
        ...
        for img_name in self.img_names:
            ...
            img.save(os.path.join(output_dir_cli_gt,f'{img_name}.png'))
    ...
    def calculateValMetric(self,opt):
        ...
        if hasattr(self,'val_dir'):
            l_input_dir.append(self.val_dir)
        if hasattr(self,'test_dir'):
            l_input_dir.append(self.test_dir)
        if len(l_input_dir) == 0:
            l_input_dir.append(self.input_dir)
        tile_size = self.tile_size[0] if hasattr(self,'tile_size') else opt['tile_size']

        from deepliif.stat import get_cell_count_metrics
        for input_dir in l_input_dir:
...
def modify_opt(d_base, d_update):
    ...
    for key, value in d_update.items():
        if key in d_res and isinstance(d_res[key], dict) and (isinstance(value, dict) and len(value) > 0):
            # recurse dicts
            d_res[key] = modify_opt(d_res[key], value)
        else:
            # replace value (including lists)
            try:
                d_res[key] = eval(value)
            except:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--subset',type=str,default=None,help='a file containing filenames one per line to indicate the files to include')
    ...
    pipeline = prepareDataPipeline(args.input_dir, args.output_dir, args.steps, args.seg_gen, options, args.subset)
    pipeline.run()
